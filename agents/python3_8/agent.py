...
TODO:
 - Better move selection logic
   - Consider each possible move and check is_safe and safe_dist to goal
     - Can also consider other heuristics to break ties and close choices
 - Better mining goal scoring
   - Calculate rough box-destroyed per turn value for each cell
     - Maybe continue considering closeness of own/opp units to affected boxes
   - No hard-cutoff at 0.01/0.02 target_range score
 - New movement goal: get as close to threatened 1hp box(es) as possible
 - Update mining goal when a bomb already placed: forbid moving into own future_fire
 - Try to prevent getting boxed in
   - For each move, consider if the closes opp unit moved toward own unit
     - Calculate affect on safe_paths
 - New movement goal: move toward opp units if they are in a small space or center cell
   - Go to left/right of center and drop bomb with < 30 turns left; take center
 - Allow more friendly fire, esp if in the lead
 - End-of-game goal: go to 2nd best cell, drop bomb, and stay on bomb / take best cell
 - Penalize moves that increase chance of getting trapped; try to trap opps

Architecture TODO:
 - process own unit moves, then opp units, then reset board, apply only opp moves, then re-process own units
   - Assume all living non-stunned units can be moved through initially?
   - idk.. don't want to skip doing the move I want because I mis-predicted their move



Pseudocodes

Threatened box goal (do something similar for updated mining goal)
for bomb_cell in cells:
  for blast_cell in blast_cells:
    if blast_cell.box and blast_cell.hp == 1:
      nearest_safe_cells = get_nearest_safe_cells(blast_cell) # min basic dist (not safe dist) to truly safe cell(s)
      for nearest_safe_cell in nearest_safe_cells:
        # These calculations can be pre-calculated once rather than re-done for each unit
        # Only save the min dist cell for each unit?
        compare own safe dists to opp safe dists


Move toward opp goal (useful for tight places inc. end-game?)
for opp_unit in opp_units:
  if opp_unit.cell.safe_dists[own_unit.id] < 8 and (own_unit.safe_paths.is_limited or opp_unit.safe_paths.is_limited):
    return opp_unit.cell
...
for cell in cells:
  for blast_cell in blast_cells:
    if blast_cell.box and blast_cell.hp == 1:
      nearest_safe_cells = get_nearest_safe_cells(blast_cell, unsafe=blast_cells)
      for nearest_safe_cell in nearest_safe_cells:
        if safe_dist <= all other units:
          cell_score += HIGH
    elif blast_cell.box and blast_cell.hp == 2:
      cell_score += LOW
    elif blast_scell.box and blast_cell.hp == 3:
      cell_score += LOW/2
sort cells by (cell_score / (safe_dist + 1 + max(5, nearest_safe_cell_dist) + 5 + nearest_safe_cell_dist))

# maybe just add to stun attack goal; points would be worth ~0.1-0.5 for one of these (no accumulate to 1)
# re-name to attack goal
# tier 1 targets: stunned units
# tier 2: units with very limited movement
# tier 3: opp goals
Threaten opp goals goal 
for opp_unit in opp_units:
  opp_goals = check for flee danger, stun attack, freeze power up goals
for cell in cells:
  for blast_cell in blast_cells:
    if blast_cell in opp_goals:
      compare 
...
def can_detonate_for_damage(board, unit, bomb_cell, diameter=None):
    ...
    own_units = [u for u in board.players[unit.player.id].units if u.hp > 0]
    opp_units = [u for u in board.players[opp_id].units if u.hp > 0]
    winning = len(own_units) > len(opp_units)
    ...
    return detonate_score > 1 or (winning and detonate_score >= -1)
...
def check_for_detonation_waiting_goal(board, unit):
    if unit.bombs and not unit.cell.future_fire_start: # Stay where you are if bomb placed and currently safe
        return unit.cell
...
def check_for_mining_goal(board, unit):
    target_range_lim = 0.02 if (40 < board.tick < 200) else 0.01
...
async def act(board, units): # TODORMA
    ...
    for unit in [u for u in units if u not in units_done]:
        detonation_waiting_goal = check_for_detonation_waiting_goal(board, unit)
        if detonation_waiting_goal:
            print(f'unit {unit.id} goal: detonation waiting {detonation_waiting_goal.x},{detonation_waiting_goal.y}')
            await do_move(board, unit, detonation_waiting_goal)
            units_done.append(unit)
